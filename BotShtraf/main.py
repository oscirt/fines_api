...
def messages(message):
    if message.text == '👮 Проверить штрафы ГИБДД':
    ...
    elif message.text == '📖 Профиль':  # Добавить инфу о штрафах
        ...
        bot.send_message(message.chat.id,
                         f'{message.from_user.first_name}, ваш профиль пользователя:\n'
                         f'Ваш id: {message.from_user.id}\n'
                         f'Ваш номер автомобиля: {user_car}\n'  # Абузы, пофиксить когда будет SQL
                         f'Ваш номер свидетельства о регистрации ТС: {user_ctc}', reply_markup=markup)
...
def carnumber(message):
    ...
    if message.text == '/start':
        ...
        message = bot.send_message(message.chat.id, 'Номер машины некорректен,\nВведите корректный номер машины')
        bot.register_next_step_handler(message, carnumber)
    else:
...
def ctc(message):
    ...
    if not re.match(pattern, ctc_number):
        message = bot.send_message(message.chat.id, 'Номер СТС некорректен,\nВведите корректный номер машины ')
        bot.register_next_step_handler(message, ctc)
    else:
...
def profile_save(message):
    ...
    button2 = types.InlineKeyboardButton('Нет', callback_data='dont_save')
...
def fines_check(message):  # Жесточайший абуз
    ...
    markup.add(types.InlineKeyboardButton('Оплатить штрафы', callback_data='payment'))  # Добавить иф штрафы есть
    bot.send_message(message.chat.id,
                     '😔 У вас есть неоплаченные штрафы! Оплатить можно сейчас или в профиле', reply_markup=markup)


@bot.callback_query_handler(func=lambda callback: True)
def callback_message(callback):  # Половина абузы
    global user_ctc
    global user_ctc1
    global user_car
    global user_car1
    ...
    elif callback.data == 'dont_save':  # Абуз
        user_car1 = ''
        user_ctc1 = ''
